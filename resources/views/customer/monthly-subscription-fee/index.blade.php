                        ...
                        <div class="card-header">
                            <div
                                ...
                                <div class="d-flex gap-2">
                                    <a href="{{ route('customer.monthly-subscription.pdf') }}" class="btn btn-info btn-sm">Download PDF</a>
                                    <a href="{{ route('customer.monthly-collection.single-page-download.pdf') }}" id="download_pdf" class="btn btn-info btn-sm">Single Page Download PDF</a>
                                </div>
                            </div>
                                    ...
                                    @if ($monthlySubscriptionFees->isNotEmpty())
                                        @foreach ($monthlySubscriptionFees as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    <a
                                                        href="{{ route('customer.monthly-subscription-fees.show', $row->id) }}">{{ $row->membershipAccount->customer->name ?? ''}}</a>
                                                </td>
                                                ...
                                                <td>
                                                    <a href="{{ route('customer.monthly-subscription-fees.show', $row->id) }}"
                                                       class="btn btn-sm btn-info">
                                                        <i class="fa fa-eye"></i>
                                                    </a>

                                                    <a href="{{ route('customer.member-account-statement.view', $row->membershipAccount->id ?? '') }}"
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#member_items').click(function(e) {
                ...
                window.location.href = "{{ route('customer.monthly-subscription-fees.index') }}?search_items=" +
                    searchItems;
            });
            ...
            $('#download_pdf').click(function(e) {
                ...
                var pdfDownloadUrl = "{{ route('customer.monthly-collection.single-page-download.pdf') }}";
