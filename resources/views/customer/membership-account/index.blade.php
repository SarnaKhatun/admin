                        ...
                        <div class="card-header">
                            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt- pb-0">
                                ...
                                <div class="ms-md-auto py-0 py-md-0">
                                        ...
                                        <a href="{{ route('customer.member-account.pdf') }}" class="btn btn-info btn-sm">Download
                                            PDF</a>

                                            <a href="{{ route('customer.membership.single-page-download.pdf') }}" id="download_pdf"
                                ...
                                <table id="" class="display table table-striped table-hover">
                                    ...
                                    @else
                                        @foreach ($membershipAccounts as $row)
                                            @php
                                                $statementId = \App\Models\MemberAccountStatement::where('membership_account_id', $row->id)->latest()->first();
                                                $balance = $statementId->balance ?? "0.00";
                                            @endphp
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $row->account_number ?? '' }}</td>
                                                <td>
                                                    <a href="{{ route('customer.monthly-subscription-fees.show', $row->id) }}"> {{ $row->name ?? '' }}</a>
                                                </td>
                                                ...
                                                <td style="width: 200px" class="najmul">
                                                    ...
                                                    <a href="{{ route('customer.membership-account.view', $row->id) }}"
                                                       class="btn btn-sm btn-info">
                                                        <i class="fa fa-eye"></i>
                                                    </a>

                                                        <a href="{{ route('customer.member-account-statement.view', $row->id) }}"
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#member_items').click(function(e) {
                ...
                window.location.href = "{{ route('customer.membership-account') }}?search_items=" +
                    searchItems;
            });
            ...
            $('#download_pdf').click(function(e) {
                ...
                var pdfDownloadUrl = "{{ route('customer.membership.single-page-download.pdf') }}";
